...
* Added: Choose which connected AI service handles spam review when more than one is configured, with a `gf_zero_spam_ai_provider` filter for developers.
* Changed: The default AI instructions now define how the AI should calibrate its confidence, for more consistent results across different AI services.
* Fixed: AI review and recovery now work with Google Gemini and other reasoning models by allowing a larger response token budget.
